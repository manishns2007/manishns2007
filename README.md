<h1 align="center">👋 Hello, I'm Manish</h1>

<p align="center">
🔐 Application Security • 🏗️ Security Engineering • 🤖 AI-Powered Security Solutions
</p>

<p align="center">
 <img src="https://readme-typing-svg.herokuapp.com?font=JetBrains+Mono&size=22&pause=1000&color=F7F7F7&center=true&vCenter=true&width=700&lines=Application+Security+Engineer;Building+Secure+Software+Systems;Threat+Modeling+%26+Security+Automation;Backend+Engineering+%26+AI+Systems;Security+Research+Enthusiast" />
</p>

---

### 🧠 About Me

I build secure backend systems, security-focused applications, and AI-powered solutions designed for real-world use.

My work focuses on:

* 🔐 Application Security
* ⚙️ Security Engineering
* 🏗️ Secure Software Development
* 🛡️ Threat Modeling
* 🤖 Security Automation
* 🌐 Backend Architecture
* 🚀 Cybersecurity Research

I am particularly interested in building tools and platforms that improve software security, investigation workflows, and system resilience.

---

### 🏆 Achievements

* 🥇 **CyberHack V4 Finalist**
* 🥈 **Import Paradox Finalist**
* 🥉 **Omi.me Indori × Ojone Hackathon Finalist**

---

### 🚀 Featured Project

#### VERITAS 

AI-assisted investigation platform designed for digital evidence analysis and intelligence generation.

**Key Features**

* Evidence ingestion and integrity verification using SHA-256
* Entity extraction and relationship mapping
* Timeline reconstruction
* AI-assisted lead generation
* Secure investigation workflow management

**Tech Stack**
Python • FastAPI • PostgreSQL • Docker • LLM Integration

---

### 🎯 Security Interests

* Application Security
* Secure Code Review
* Threat Modeling
* Security Automation
* Vulnerability Research
* Secure System Design
* Cloud Security

---

### 🛡️ Cybersecurity, Backend & DevOps Toolkit

| Category                 | Technologies                                |
| ------------------------ | ------------------------------------------- |
| 🛡️ Application Security | Burp Suite, OWASP Testing Methodology       |
| ⚙️ Security Engineering  | Python, Security Automation, Custom Tooling |
| 🌐 Backend Development   | Python, Node.js, REST APIs                  |
| 🔬 Security Research     | Vulnerability Analysis, Threat Modeling     |
| 📡 Network Security      | Wireshark, Nmap, OpenVAS                    |
| ☁️ Cloud & DevOps        | AWS, Azure, Docker, GitHub Actions, CI/CD   |
| 💻 Platforms             | Linux, Kali Linux, Windows                  |

---

### 🧰 Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,cpp,c,java,js,linux,mysql,git,github,docker,aws&perline=7" />
</p>

---

### 📊 Competitive Programming Stats

<p align="center">
  <img src="https://leetcard.jacoblin.cool/manishns2007?theme=dark&font=JetBrains%20Mono" />
</p>

---

### 🔐 Security Platforms

<p align="center">
  <a href="https://tryhackme.com/p/nsmanish2007">
    <img src="https://img.shields.io/badge/TryHackMe-red?style=for-the-badge&logo=tryhackme&logoColor=white"/>
  </a>

  <a href="https://app.hackthebox.com/">
    <img src="https://img.shields.io/badge/Hack%20The%20Box-111927?style=for-the-badge&logo=hackthebox&logoColor=9FEF00"/>
  </a>
</p>

---

### 📈 GitHub Stats & Activity

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=manishns2007&theme=tokyonight" />
</p>

---

### 🐍 GitHub Contribution Snake

<p align="center">
  <img src="https://raw.githubusercontent.com/manishns2007/manishns2007/output/github-contribution-grid-snake.svg" />
</p>

---

### 🧠 Current Focus

* Application Security
* Secure Code Review
* Security Tool Development
* Backend Architecture & System Design
* AI-Powered Security Workflows
* Cloud Security Fundamentals

---

### ☕ Let's Connect!

<p align="center">
  <a href="mailto:nsmanish2007@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>

  <a href="https://www.instagram.com/manish.pog/">
    <img src="https://img.shields.io/badge/Instagram-black?style=for-the-badge&logo=instagram"/>
  </a>

  <a href="https://tryhackme.com/p/nsmanish2007">
    <img src="https://img.shields.io/badge/TryHackMe-red?style=for-the-badge&logo=tryhackme&logoColor=white"/>
  </a>
</p>

---

> *"Build systems that are secure by default and scalable by design."*

